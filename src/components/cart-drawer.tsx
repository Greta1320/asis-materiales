...
export function CartDrawer({ open, onClose }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <aside
        className={`fixed top-0 right-0 h-full z-[60] flex flex-col transition-transform ${open ? "translate-x-0" : "translate-x-full"}`}
        style={{ width: "min(410px, 100%)", background: "var(--color-surface)", boxShadow: "-10px 0 40px rgba(0,0,0,.2)" }}
      >
        {/* Cabecera */}
          ...
          {items.length === 0 ? (
            ...
            items.map(({ product, qty }) => (
              <div key={product.id} className="flex gap-3 items-center py-3 border-b" style={{ borderColor: "var(--color-line)" }}>
                {/* Miniatura */}
                <div className="w-13 h-13 rounded-lg shrink-0 overflow-hidden grid place-items-center" style={{ background: "var(--color-surface-2)" }}>
                  {product.image_url ? (
                    <Image src={product.image_url} alt="" width={52} height={52} className="w-full h-full object-cover" />
                  ) : (
                ...
                {/* Qty */}
                <div className="flex items-center rounded-lg border overflow-hidden shrink-0" style={{ borderColor: "var(--color-line)" }}>
                  <button
          ...
          {items.length > 0 && (
            <div className="flex flex-col gap-2">
              <a
                ...
                className="w-full flex items-center justify-center gap-2 rounded-xl text-white font-bold text-base py-3.5 no-underline transition-colors"
                style={{ background: "var(--color-wsp)" }}
                onMouseOver={(e) => (e.currentTarget.style.background = "var(--color-wsp-hover)")}
                onMouseOut={(e) => (e.currentTarget.style.background = "var(--color-wsp)")}
              >
              ...
              <button
                onClick={clear}
                className="text-xs text-center py-1 border-0 bg-transparent"
                style={{ color: "var(--color-ink-soft)" }}
              >
                Vaciar pedido
              </button>
