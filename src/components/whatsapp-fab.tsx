...
export function WhatsAppFAB({ onOpenCart }: WhatsAppFABProps) {
  ...
  return (
    <button
      onClick={onOpenCart}
      className="fixed bottom-5 right-5 z-40 rounded-full flex items-center gap-2 font-semibold text-[15px] text-white px-5 py-3.5 transition-all duration-200 hover:scale-105 active:scale-95"
      style={{ background: "var(--color-wsp)", boxShadow: "var(--shadow-md)" }}
      aria-label={count > 0 ? `Ver pedido — ${formatPrice(total)}` : "Ver pedido"}
    >
